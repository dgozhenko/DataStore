...
class ProtoStoreImpl @Inject constructor(@ApplicationContext context: Context): ProtoStore {
    private val dataStore: DataStore<FilterOption> = context.createDataStore(fileName = "courses.pb", serializer = FilterSerializer())

    override val filterFlow: Flow<FilterOption> = dataStore.data.catch { exception ->
